...
function generateTripsMigrations() {
  ...
  chunks.forEach((chunk, idx) => {
    ...
    const values = chunk.map(t =>
      `(${sqlEscape(PROVIDER_ID)}, ${sqlEscape(t.trip_id)}, ${sqlEscape(t.route_id)}, ${sqlEscape(t.service_id)}, ${sqlEscape(t.trip_headsign)}, ${sqlEscape(t.direction_id || null)}, ${sqlEscape(t.block_id)}, ${sqlEscape(t.shape_id)})`
    );
